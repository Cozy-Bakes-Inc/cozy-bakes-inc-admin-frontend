...
import type {
  ...
  PickupListApiResponse,
  SinglePickupApiResponse,
} from "@/types/main/pickup-locations";
import { baseAPI } from "..";
...
export const shopListAPI = async (
  ...
  return await baseAPI<PickupListApiResponse>(
    "GET",
    `/shop/list?${params.toString()}`,
  );
};
export const singleShopAPI = async (slug: string) =>
  await baseAPI<SinglePickupApiResponse>("GET", `/shop/${slug}/view`);
