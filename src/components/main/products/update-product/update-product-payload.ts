...
export function buildPartialUpdatePayload(
  ...
  if (dirtyFields.category)
    payload.append("sub_category_id", values.category.trim());
  if (dirtyFields.description)
    payload.append("description", values.description.trim());
  if (dirtyFields.ingredients)
    payload.append("description_ingredient", values.ingredients.trim());
  if (dirtyFields.allergens)
    payload.append("description_allergens", values.allergens.trim());
  ...
  if (isPricingDirty(dirtyFields)) {
    ...
    if (values.pricingType === "perUnit") {
      ...
      payload.append("prices[per_unit][0][label]", `Single`);
      payload.append(
        "prices[per_unit][0][meta][parcel][length]",
        parcel.length,
      );
      payload.append("prices[per_unit][0][meta][parcel][width]", parcel.width);
      payload.append(
        "prices[per_unit][0][meta][parcel][height]",
        parcel.height,
      );
      payload.append(
      ...
      payload.append(
        "prices[per_unit][0][meta][parcel][weight]",
        parcel.weight,
      );
      payload.append(
    ...
    if (values.pricingType === "comboDeal") {
      values.comboDeals.forEach((deal, i) => {
        ...
        const label = qty === 1 ? `Single` : `Pack of ${qty}`;
