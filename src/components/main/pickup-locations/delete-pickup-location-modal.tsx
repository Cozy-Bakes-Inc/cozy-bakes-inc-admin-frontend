...
import { deleteShop } from "@/services/mutations";

interface DeletePickupLocationModalProps {
  open: boolean;
  slug: string | null;
  name: string;
  onClose: () => void;
...
export function DeletePickupLocationModal({
  open,
  slug,
  name,
  onClose,
}: DeletePickupLocationModalProps) {
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true);
    const result = await deleteShop(slug);
    setIsDeleting(false);

    if (result?.ok) {
      toast.success(result.message || "Pickup location deleted successfully");
      await Promise.all([
    ...
    toast.error(result?.message || "Failed to delete pickup location");
  }
    ...
    >
      <div className="flex flex-col items-center justify-center px-7 py-10 text-center">
        ...
        <p className="mt-2 max-w-115.75 text-[16px] font-medium leading-6 text-muted-text">
          ...
          <span className="font-semibold text-primary">{name}</span>? This
        ...
        <div className="mt-6 grid w-full gap-4 sm:grid-cols-2">
          ...
          >
            {isDeleting ? <Loader /> : "Delete Location"}
          </Button>
        </div>
      </div>
    </Modal>
